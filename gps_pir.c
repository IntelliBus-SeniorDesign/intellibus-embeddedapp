...
/********************************************************************
* Author: Yue Pan
* Last Date Modified: 09/29/21
* Multi-threaded application running passenger count and GPS 
*********************************************************************/
...
static void gps_handler(const struct device *dev, struct gps_event *evt)
{
	...
	case GPS_EVT_AGPS_DATA_NEEDED:
		...
		break;
	case GPS_EVT_PVT:
                // only print GPS info at a 20 second interval
                if ((k_uptime_get() - start_search_timestamp) % 20 == 0) {  
...
K_THREAD_DEFINE(pir_thread_id, STACKSIZE, pir_thread, NULL, NULL, NULL,
                PRIORITY, 0, 0);

void main(void)
{
        LOG_INF("Starting GPS + PIR sensors program!\n"); // this LOG_INF can only be displayed once
	int err;
...
K_THREAD_DEFINE(gps_thread, STACKSIZE, main, NULL, NULL, NULL,
                PRIORITY, 0, 0);
