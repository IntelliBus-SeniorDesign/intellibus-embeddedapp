...
;
    ...
    int gnxx_nmea_mask_set_retval = nrf_modem_gnss_nmea_mask_set(NRF_MODEM_GNSS_NMEA_RMC_MASK |
					 NRF_MODEM_GNSS_NMEA_GGA_MASK |
					 NRF_MODEM_GNSS_NMEA_GLL_MASK |
					 NRF_MODEM_GNSS_NMEA_GSA_MASK |
					 NRF_MODEM_GNSS_NMEA_GSV_MASK);
    
	if (gnxx_nmea_mask_set_retval != 0) {
		printk("[GPS] Failed to set GNSS NMEA mask, retval %d \n", gnxx_nmea_mask_set_retval);
		return -1;
	}
...
static int shadow_update(bool version_number_include)
{
    ...
    */
	...
	err += json_add_number(reported_obj, "timestamp", message_ts);
    err += json_add_number(reported_obj, "deviceID", 102);
    err += json_add_number(reported_obj, "pc", passenger_count);
    err += json_add_str(reported_obj, "coordinates", "33.7815621350692, -84.40740388920352");
    
    err += json_add_number(reported_obj, "latitude", 33.7815621350692);
    err += json_add_number(reported_obj, "longitude", -84.40740388920352);
    
    // 33.78152079641171, -84.39159655411603
...
//    printk("[GPIO] start gpio_sensors_count thread\n");
...
//    bool entering   = false; 
//    const int max_iterations_delay = 150;
...
void gpio_sensors_count(void) {
    ...
    while (1) {
        ...
        else if (sense1High && sense2High)
        {
            ...
            printk("The number of passengers is: %d\n", passenger_count); 
        }
        k_sleep(K_MSEC(20));  
    }
}
...
void main(void)
{
    ...
    k_thread_create(
           &gps_thread,                     // thread pointer
           gps_thread_stack, 8192,            // stack pointer, stack size
           (k_thread_entry_t) gps_main,   // entry point
           NULL, NULL, NULL,                        // param 1, param 2, param 3
           K_PRIO_COOP(7), 0, K_NO_WAIT             // priority, options, delay
    );
    
    printk("[MAIN] GPS thread created\n");
